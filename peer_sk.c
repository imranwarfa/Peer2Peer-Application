...
void	server_download(int s_sock, int d_sock, int p_sock, char p_host[], int p_port, struct sockaddr_in p_server, char *name, struct sockaddr_in reg_addr)
{
	...
	PDU dpdu, rpdu, spdu, fpdu;
	char buf[BUFLEN]; char c_name[BUFLEN];
	char contentname[20], filename[20];
	int n, alen;
	...
	if (rpdu.type == 'A'){ // Registered file exists
		...
		if (connect(d_sock, (struct sockaddr *)&p_server, sizeof(p_server)) == -1){
			...
			n = read(0, fpdu.data, 100);
			fpdu.data[n-1] = '\0';
			strcpy(filename, fpdu.data);
			write(d_sock, &fpdu, BUFLEN); // Sending file name
			int f_data = 0;
			FILE *fp;
			fp = fopen(filename, "w");
			...
			printf("Enter the name of the content of the downloaded file\n");
			bzero(buf,BUFLEN);
			n = read(0, buf,100);
			buf[n-1] = '\0';
			strcpy(c_name,buf);
			printf("The content name is: %s", &c_name);
			...
			write(s_sock, &c_name, BUFLEN);				// content name
...
void deregistration(int s_sock, int p_sock, char *name)
{
	...
	/*printf("Enter the name of the file from the content name selected:\n");
	bzero(buf_f,BUFLEN);
	n = read(0, buf_f,100);
	buf_f[n-1] = '\0'; */
	...
	spdu.type = 'T';
	strcpy (spdu.data, name); 
	write(s_sock, &spdu, BUFLEN); // write username to server
	write(s_sock, &buf, BUFLEN);  // write content name to server
	//write(s_sock, &buf_f, BUFLEN);// write file name to server
	read(s_sock, &rpdu, BUFLEN);  // read result from server
	printf("%s\n", rpdu.data);
}
...
void registration(int s_sock, int p_sock, char *name, struct sockaddr_in reg_addr)
{
	/* Create a TCP socket for content download 
	   ...
	   Update nfds;	
	   Name refers to the users username		
	*/
	char buf[BUFLEN]; char c_name[BUFLEN];
	PDU spdu;
	FILE *fp;
	...
	if (fp == NULL) {
		...
		n = read(s_sock, &spdu, BUFLEN); // Check for error message and display to user if content is registered or not
		if (spdu.type == 'A')
			printf("Content has been registered.\n");
		
		else if (spdu.type == 'E')
			printf("Content name has already been registered. Please pick a different name.\n");
